					...
					if (param['name'] == 'JOB' && param['value'] == job_id) {
						mergeatron.mongo.jobs.findOne({ _id: job_id }, function(error, job) {
							if (job['status'] == 'new') {
								...
								mergeatron.emit('build_started', job_id, job, build['url']);
							}

							if (job['status'] != 'finished') {
								if (build['result'] == 'FAILURE') {
									mergeatron.mongo.jobs.update({ _id: job_id }, { $set: { status: 'finished' } });
									mergeatron.emit('build_failed', job_id, job, build['url'] + 'console');
								} else if (build['result'] == 'SUCCESS') {
									mergeatron.mongo.jobs.update({ _id: job_id }, { $set: { status: 'finished' } });
									mergeatron.emit('build_succeeded', job_id, job, build['url']);
								}
							}
						});
					}
